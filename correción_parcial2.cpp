...
public:
	...
	//Ordenar las cartas de un jugador, primero por color y luego por numero
        Nodo<Tarjeta*>* move =  ptrc;
            while(move->get_next() != NULL){
                if(move->get_dato()->get_color() > move->get_next()->get_dato()->get_color()){
                    if(move->get_dato()->get_numero() > move->get_next()->get_dato()->get_numero()){
                        move->set_next(move->get_next()->get_next());
...
public:
    ...
    int calc_points(){ //Recorra la lista de tarjetas y sume los números de las tarjetas
        ...
        if(ptrc != NULL){
            while(ini != NULL){
                sum += ini->get_dato()->get_numero();
                if(ini->get_dato()->get_numero() == 0){
                    sum = sum + 20;
                }
                ini = ini->get_next();
            }
    ...
    string to_string(){
        ...
        if(ptrc == NULL){
            s =  s + "La lista secundaria estÃ¡ vacía uwun't. ";
        }
        return s;
    }
...
public:
    ...
    void sort(){
        //Implemente un método de ordenamiento
...
int main(){
    ...
    //Muestre el puntaje de los jugadores, en orden de id
    // for(int i = 0; i < num_colores; i++){
    //     std::cout << "jugador " << i << " tiene " << std::to_string<< std::endl;
    // } 
